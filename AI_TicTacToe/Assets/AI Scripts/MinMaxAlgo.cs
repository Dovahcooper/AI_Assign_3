...
public class MinMaxAlgo : MonoBehaviour
{
    ...
    //this returns a Vector3Int so I can send scores, rows, *and* columns up the chain until we exit the recursion
    //the y and z values correspond to i and j respectively when sending the
    public Vector3Int minimax(int[,] logicArray, int playerType)
    {
        ...
        int otherPlayer;    //this part is a lot easier if you use 1 and -1, since you can just multiply playerType by -1 to get otherPlayer
                            //honestly, I'd've preferred it that way, but I was dumb and did the programming for tictactoe before I got to writing the minimax algorithm
        if (playerType == 2)
        {
            otherPlayer = 1;
        }
        ...
        for (int i = 0; i < 3; i++)
        {
            for (int j = 0; j < 3; j++)
            {
                if (logicArray[i, j] == 0) //if the current selection is a blank square, we start running calculations
                {
                    ...
                    Vector3Int moveScore = minimax(tempBoard, otherPlayer); //wesend that tempBoard into the minimax function again to be recursively called into "almost" infinity
                    ...
                    if (moveScore.x > score)
                    {
                        ...
                        score = moveScore.x;
                        //and we grab the array values that got us a better score so we can eventually choose that path outside the recursion
                        row = i;
                        col = j;
                    }
                }
